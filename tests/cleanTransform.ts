...
describe('clean transform passing report', function () {
  ...
  describe('no clean transform check', function () {
    before('load schema', async function () {
      try {
        await v.schema.loadFromFileSystem('tests/schemas/clean-transform/clean-transform-not-required.json');
      } catch (err) {
  ...
  describe('root node has clean transform', function () {
    before('load schema', async function () {
      try {
        await v.schema.loadFromFileSystem('tests/schemas/clean-transform/clean-transform-required.json');
      } catch (err) {
...
describe('root node clean transform failing report', function () {
  ...
  before('load model', async function () {
    try {
      await v.model.loadFromFileSystem(['tests/models/blender-default-cube-bad-transform.glb']);
    } catch (err) {
