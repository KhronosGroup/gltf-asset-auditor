...
argv.forEach(arg => {
  ...
  } else if (['.bin', '.jpg', 'jpeg', '.ktx', 'ktx2', '.png', 'webp'].includes(arg.slice(-4))) {
...
if (outputIndex > 0 && outputJsonFilename == '' && outputCsvFilename == '') {
  ...
  if (glbFilename) {
    outputJsonFilename =
      glbFilename.substring(glbFilename.lastIndexOf(path.sep) + 1, glbFilename.length - 4) +
      '-3DQC-' +
      timestamp +
      '.json';
  } else if (gltfFileName) {
    outputJsonFilename =
      gltfFileName.substring(gltfFileName.lastIndexOf(path.sep) + 1, gltfFileName.length - 5) +
      '-3DQC-' +
      timestamp +
      '.json';
  }
...
if (gltfFileName && !gltfSupportingFiles.map(i => i.slice(-4)).includes('.bin')) {
  printIfNotQuiet(chalk.red('Using a .gltf file also needs to include a .bin file plus external textures'));
  exit(1);
}

// Print a message at the start of the program
const printWelcomeMessage = version => {
  printIfNotQuiet(chalk.green('-- glTF Asset Auditor --'));
...
try {
  ...
  await auditor.schema.loadFromFileSystem(schemaFilename);

  // 2. Load GLB or GLTF + files
  // If a glb path exists, use it, otherwise use the gltf and supporting files
  let fileList = glbFilename ? [glbFilename] : [gltfFileName, ...gltfSupportingFiles];
  await auditor.model.loadFromFileSystem(fileList);

  // 3. Load Product Info (optional)
  if (productInfoFilename) {
    await auditor.productInfo.loadFromFileSystem(productInfoFilename);
  }
  ...
  auditor.report.getItems().forEach(item => {
  ...
  printIfNotQuiet(chalk.magenta('==== Audit Report ===='));
  ...
  printIfNotQuiet('Total Time: ' + ((Date.now() - startTime) / 1000).toFixed(3) + ' seconds.');
  if (outputCsvFilename) {
    await fs.writeFileSync(outputCsvFilename, auditor.getReportCsv(), {}, err => {
      if (err) {
  ...
  if (outputJsonFilename) {
    await fs.writeFileSync(outputJsonFilename, auditor.getReportJson(), {}, err => {
      if (err) {
        printIfNotQuiet('there was an error writing the csv file');
      }
    });
